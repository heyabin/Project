...
<s:if test="#session.pb==null">
	<c:redirect url="findAll_BookType.action"></c:redirect>
</s:if>
...
<script>
	...
	function doChangeRows(){
		...
		window.location="findAll_BookType.action?rows="+rows;
	...
	function doChangePage(){
		...
		window.location="findAll_BookType.action?page="+page;
...
<body>
<p align="center"><strong>图书类别管理</strong></p>
...
<tr >
  ...
  <s:iterator value="#session.pb.pagelist" var="bookType">
  <tr align="center">
   ...
   <td>
     <a href="<%=request.getContextPath()%>/delById_BookType.action?bookTypeId=${bookType.bookTypeId}" onclick="delo(return doel('${student.stuName}'))">删除</a>
     <a href="<%=request.getContextPath()%>/findById_BookType.action?bookTypeId=${bookType.bookTypeId}">修改</a></td>
  </tr>
  </s:iterator>
  <!-- 每页记录数不够rows条，按空行补齐 -->
...
<form id="form1" name="form1" method="post" action="">
  <table width="700" border="1" align="center" cellpadding="1" cellspacing="0">
    <tr align="center" bgcolor="#CCCCCC">
      <td width="44">
      		<c:if test="${pb.page>1}">
      			<a href="<%=request.getContextPath()%>/findAll_BookType.action?page=1">
      		</c:if>
      		首页</a></td>
      <td width="44">
      		<c:if test="${pb.page>1}">
      			<a href="<%=request.getContextPath()%>/findAll_BookType.action?page=${pb.page-1}">
      		</c:if>	
      			上页</a></td>
      <td width="44">
      ...
      <td width="44">
      		<c:if test="${pb.page<pb.maxpage}">	
      			<a href="<%=request.getContextPath()%>/findAll_BookType.action?page=${pb.maxpage}">
      		</c:if>		
      			末页</a></td>
      <td width="220">每页
...
<p>&nbsp;</p>
<p align="center"><strong><a href="saveBookType.jsp">返回添加</a></strong></p>
</body>
</html>